...
var removeChildren = function removeChildren(tree, node) {
  var siblings = node.parent.children; //找到他的兄弟姐妹
  //寻找需要删除的node的下标

  var index = 0; //表示肯定存在这个下标
...
var find = function find(tree, node) {
  //在tree中找node这个节点
  if (tree === node) {
    return tree; //如果在tree中没有找到节点node,就继续到它的儿子中找
  } else if (tree.children) {
    for (var i = 0; i < tree.children.length; i++) {
      //遍历所有儿子
      var result = find(tree.children[i], node); //如果第i个儿子 = node,就记为result

      if (result) {
        return result;
      } //如果result存在，就返回result
